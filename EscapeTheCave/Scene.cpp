...
Scene::~Scene() {
    for (auto obj : statics) delete obj;
    for (auto obj : moving) delete obj;
}

void Scene::Add(Object* obj, int type) {
    if (type == STATIC) statics.push_back(obj);
    else moving.push_back(obj);
}

void Scene::Remove(Object* obj, int type) {
    if (type == STATIC) statics.remove(obj);
    else moving.remove(obj);
}

void Scene::Remove() {
    if (processing == STATIC) statics.remove(*it);
    else moving.remove(*it);
}
...
Object * Scene::Next() {
    if (its != statics.end()) {
        ...
        if(itm == moving.end()) return nullptr;
